...
bool LynxarmHLD::sendSingleServoToPosition(
  uint8_t channel,int64_t degrees, uint64_t time,
...
{              /// if action is used for single servo only then
  ...
  } else if (pulse_width > 180 && pulse_width < 500) {
    RCLCPP_ERROR(
      rclcpp::get_logger(
        "rclcpp"),
      "Invalid pulse width value or angle in degrees. Please enter a degree between 0 and 180 or a PWM between 500 and 2500");
    return false;
  }
...
int LynxarmHLD::convertToDegrees(int pwm)
{
  ...
  // Map the valid_pwm to the degree range of 0 to 180
...
bool LynxarmHLD::sendPredefinedPosition(predefined_positions position)
{
  switch (position) {
    case 0:
      ...
      lowLevelDriver.queueCommand(
        5,
        convertToSafePWM(WRIST_ROTATE_PARK, WRIST_ROTATE_LEFT, WRIST_ROTATE_RIGHT));
      RCLCPP_DEBUG_STREAM(
        this->get_logger(),
        "command is extended, currently: " << lowLevelDriver.getCommand() << std::endl);
    ...
    case 1:
      ...
      lowLevelDriver.queueCommand(
        5,
        convertToSafePWM(WRIST_ROTATE_READY, WRIST_ROTATE_LEFT, WRIST_ROTATE_RIGHT));
      RCLCPP_DEBUG_STREAM(
        this->get_logger(),
        "command is extended, currently: " << lowLevelDriver.getCommand() << std::endl);
      lowLevelDriver.sendQueuedCommands();
    ...
    case 2:
      ...
      lowLevelDriver.queueCommand(
        5,
        convertToSafePWM(WRIST_ROTATE_STRAIGHTUP, WRIST_ROTATE_LEFT, WRIST_ROTATE_RIGHT));
      RCLCPP_DEBUG_STREAM(
        this->get_logger(),
        "command is extended, currently: " << lowLevelDriver.getCommand() << std::endl);
      lowLevelDriver.sendQueuedCommands();
...
{
  RCLCPP_INFO(
    this->get_logger(), "STATE:{MOVING} Received goal request with values: servo: %d goal position in degrees: %ld, speed in %ld, time %ld ",
...
void LynxarmHLD::execute_servo(const std::shared_ptr<GoalHandleServo> goal_handle)
{
  ...
  if (rclcpp::ok()) {
    ...
    goal_handle->succeed(result);
    RCLCPP_INFO(
      this->get_logger(), "STATE:{MOVED}, Goal succeeded, servo %d is now in position %ld, in %ld ms",goal->servo, goal->position,
       result->end_time);
  }
